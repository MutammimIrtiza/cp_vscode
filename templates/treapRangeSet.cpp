...
struct Treap {
    ...
    void push(Node* t){
        if(!t) return;
        if(t->lazy_set != INT32_MIN){
            t->val = t->lazy_set;
            t->sum = t->lazy_set * t->sz;
            t->mn = t->mx = t->lazy_set;
            if(t->l) t->l->lazy_set = t->lazy_set;
            if(t->r) t->r->lazy_set = t->lazy_set;
            t->lazy_set = INT32_MIN;
        }
